...
function um_bbpress_user_add_tab( $tabs ) {
	...
	$tabs['forums']['subnav']         = array(
		...
		'favorites'     => __( 'Favorites', 'um-bbpress' ) . '<span class="um-profile-favorites">' . count( bbp_get_user_favorites_topic_ids( $user_id ) ) . '</span>',
		'subscriptions' => __( 'Subscriptions', 'um-bbpress' ) . '<span class="um-profile-subscriptions">' . UM()->bbPress_API()->user_subscriptions_count( $user_id ) . '</span>',
	);
