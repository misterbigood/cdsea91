...
*/
    ...
    function protect($isAdmin = false, $userGroup = false)
    {
        ...
        $this->errorCharset = $globalConfig['errorCharset']; session_start();
...
*/
    ...
    function checkSession()
    {
        if (!$this->hasCookie() && $this->accNoCookies && (@$_POST['action'] != "login" || @$_GET)) {
            ...
            else if (@$_SESSION['userID'] && @$_SESSION['sessionID']) {
...
*/
    ...
    function makeLogout()
    {
        ...
        if ($this->enblRemember && isset($_COOKIE[$this->cookieRemName]) && isset($_COOKIE[$this->cookieRemPass])) {
            setcookie($this->cookieRemName, $_COOKIE[$this->cookieRemName], time()); setcookie($this->cookieRemPass, $_COOKIE[$this->cookieRemPass], time());
        } session_destroy(); header("Location: " . $this->logoutUrl);
    }
    ...
    function checkRemember()
    {
        if ($this->enblRemember && isset($_COOKIE[$this->cookieRemName]) && isset($_COOKIE[$this->cookieRemPass])) {
            ...
            if ($_COOKIE[$this->cookieRemName] == $data['userName'] && $_COOKIE[$this->cookieRemPass] == $data['userPass']) {
                ...
                $_SESSION['userID'] = $data['ID']; setcookie($this->cookieRemName, $_COOKIE[$this->cookieRemName], time() +(60*60*24*$this->cookieExpDays)); setcookie($this->cookieRemPass, $_COOKIE[$this->cookieRemPass], time() +(60*60*24*$this->cookieExpDays));
                $this->checkSession();
            }
        }
        else {
    ...
    function checkLogin()
    {
        ...
        if ($action == "login" && ($userName || $userPass)) {
            if ($userName == $data['userName'] && $userPass == $data['userPass']) {
				...
				if($data['isAdmin'] == 1) {
					$this->isUserAdmin = $_SESSION['isUserAdmin'] = true;
				}
				else $this->isUserAdmin = $_SESSION['isUserAdmin'] = false;
                if ($this->enblRemember && @$_POST['userRemember'] == "yes") {setcookie($this->cookieRemName, @$_POST['userName'],time() +(60*60*24*$this->cookieExpDays)); setcookie($this->cookieRemPass, md5(@$_POST['userPass']) ,time() +(60*60*24*$this->cookieExpDays));
                }
                $this->checkSession();
            }
        }
...
*/
    ...
    function makeErrorHtml()
    {
        ...
        if ($this->errorCssUrl != "") {    // FIND IF WE ARE INTO AP DIRECTORY TO REQUEST THE CORRECT CSS AND LOGIN FILES
            ...
            if($AP == 'YES') {
                ...
                $out .= "<link rel=\"stylesheet\" type=\"text/css\" href=\"AP/" . $this->errorCssUrl . "\">\n";
            }
            ...
            if($AP == 'YES') {    // IF WE'RE IN AP DIRECTORY
...
*/
    ...
    function showPage()
    {
		$this->isUserAdmin = $_SESSION['isUserAdmin'];
        $this->showPage = true;
    }
