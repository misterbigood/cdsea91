...
$ID = $_GET['ID'];
if(!isset($_GET['alerteMsg'])) { // SI 1er POST, sans msg d'erreur
	$qry = "SET NAMES 'utf8'";
	$db = new MySQL();
	$db->Open();
	$db->query($qry);
	$qry = "SELECT * FROM offres_emploi WHERE ID='$ID'";
	$db = new MySQL();
	$db->Open();
	$db->query($qry);
	$row = $db->Row();
	...
	$_SESSION['rubrique'] = $row->rubrique;
    $_SESSION['html'] = $row->html;
    $_SESSION['date_publication'] = $row->date_publication;
...
$form->startFieldset('Modifier une offre d\'emploi');
...
var js = function(divAjax) {
	...
	$('#textareaIntro').compteurMotsCaracteres(150);
	$(".info a").tooltip();
    $('.tinyMce').tinymce({
