...
$(document).ready(function(){
	...
	$("#search").focus(function () {
		...
		$('#lib-title').empty();
		$('#lib-title').append('Recherche... <a href="" id="clear-search">annuler</a>');
	...
	$(document).on('click', 'a#clear-search', function () {
		...
		$('#gallery-images').append('<div id="ajax-loader-div"><img src="bootstrap/img/ajax-loader.gif" alt="Chargement..." class="ajax-loader"></div>');
		$.getJSON('lib.php' + '?dummy=' + new Date().getTime(),{}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').empty();
				$('#gallery-images').append('<center>Aucune image dans la bibliothèque.</center>');
			}
		});
		return false;
	});
	...
	$("#search").keyup(function(event) {
			...
			if(results.length > 0){
				...
				$('#gallery-images').append('<center>Aucune image ne correspond à cette recherche.</center>');
			}
	...
	$("#get-recent").bind("click", function () {
		$('#recent-images').empty();
		$('#recent-images').append('<div id="ajax-loader-div"><img src="bootstrap/img/ajax-loader.gif" alt="Chargement..." class="ajax-loader"></div>');
		$.getJSON('recent.php',{}, function(returned){ 
			if(returned.success == 1){
				...
				$('#recent-images').empty();
				$('#recent-images').append('<center>Aucune image récente trouvée.</center>');
			}
		});
	});
	...
	$("#refresh").bind("click", function () {
		...
		$.getJSON('lib.php' + '?dummy=' + new Date().getTime(),{path: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').empty();
				$('#gallery-images').append('<center>Aucune image dans le répertoire.</center>');
			}
		});
	...
	$(document).on('click', '#newfolder_btn', function () {
		if($('#newfolder_name').val() == ""){
			alert('svp indiquez un nom pour le nouveau répertoire');
			return false;
		}
		
		$('#new-folder-msg').empty();
		$('#new-folder-msg').append('Création...&nbsp;&nbsp;&nbsp;');
		...
		$.getJSON('new_folder.php' + '?dummy=' + new Date().getTime(),{path: $("#refresh").attr("rel"), folder: $('#newfolder_name').val()}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').append(returned.html);
				$('#new-folder-msg').empty();
				$('#new-folder-msg').append('<span style="color: green;">Terminé...&nbsp;&nbsp;&nbsp;</span>');
				setTimeout(function(){ $('#new-folder-msg').empty() }, 5000);
			}else{
				$('#new-folder-msg').empty();
				$('#new-folder-msg').append('<span style="color: red;">Erreur...&nbsp;&nbsp;&nbsp;</span>');
				setTimeout(function(){ $('#new-folder-msg').empty() }, 5000);
				if(returned.msg != ""){
					alert(returned.msg);
				}
	...
	$(document).on('click', 'a.delete-file', function () {
		...
		var r=confirm("Etes-vous sûr de vouloir effacer ce fichier ?");
		if(r==false){
			return false;
		}
		$(this).parent().parent().empty().append('<p>Suppression...</p>');
	...
	$(document).on('click', 'a.delete-folder', function () {
		var content = $(this).parent().parent().html();
		var the_parent = $(this).parent().parent();
		var r=confirm("Etes-vous sûr de vouloir effacer ce répertoire et tout son contenu ?");
		if(r==false){
			return false;
		}
	...
	$(document).on('click', 'a.change-folder', function () {
		...
		var content = $(this).parent().parent().html();
		var the_parent = $(this).parent().parent();
		var r=prompt("SVP indiquez un nouveau nom",current_value);
		if(r==null || r==""){
			return false;
		}
		...
		$(this).parent().parent().empty().append('<p>Enregistrement ...</p>');
	...
	$(document).on('click', 'a.change-file', function () {
		...
		var r=prompt("SVP indiquez le nouveau nom",current_file_name);
		if(r==null || r==""){
			return false;
		}
	...
	$(document).on('click', '#refresh-dirs', function () {
		...
		$('#select-dir-msg').append('Chargement...&nbsp;&nbsp;&nbsp;');
	...
	$(document).on('change', '#select-dir', function () {
		$('#select-dir-msg').empty();
		$('#select-dir-msg').append('Envoi en cours ...&nbsp;&nbsp;&nbsp;');
		
		$.getJSON('set_upload_directory.php' + '?dummy=' + new Date().getTime(),{path:$(this).val() }, function(returned){ 
			if(returned.success == 1){
				$('#select-dir-msg').empty();
				$('#select-dir-msg').append('<span style="color: green;">Terminé ...&nbsp;&nbsp;&nbsp;</span>');
				setTimeout(function(){ $('#select-dir-msg').empty() }, 5000);
			}
		});
		return false;
	...
	$(document).on('click', 'a.lib-folder', function () {
		...
		$.getJSON('lib.php' + '?dummy=' + new Date().getTime(),{path: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').empty();
				$('#gallery-images').append('<center>Aucune image dans ce dossier.</center>');
			}
		});
				...
				<div class="span12" style="margin-top: 20px;">
					...
					<div class="tabbable tabs-left">
						<ul class="nav nav-tabs" id="myTab">
							<li><a href="#tab1" data-toggle="tab"><i class="icon-globe"></i> Ajouter depuis l'URL</a></li>
							<?php if(CanAcessLibrary()){?>
							<li><a href="#tab2" data-toggle="tab" id="get-lib"><i class="icon-folder-open"></i> Ajouter depuis la bibliothèque</a></li>
							<?php }?>
							<?php if(CanAcessUploadForm()){?>
							<li><a href="#tab3" data-toggle="tab"><i class="icon-upload"></i> Uploader un fichier</a></li>
							<?php }?>
							<li><a href="#tab4" data-toggle="tab" id="get-recent"><i class="icon-time"></i> Récent</a></li>
						</ul>
...
<p>
<input type="text" id="title" name="title" value="<?php echo $title;?>" placeholder="Titre" title="Title">
</p>
...
<br/>
<p>
<input type="text" id="width" name="width" class="input-small dimensions" placeholder="Largeur" title="Width" value="<?php echo $width;?>"> &times; <input type="text" id="height" name="height" class="input-small dimensions" placeholder="Hauteur" title="Height" value="<?php echo $height;?>"> <br/><input type="checkbox" id="constrain" name="constrain" checked="checked"> Conserver les proportions
</p>
<br/>
<p>
<select id="float" name="float">
<option value="">Alignment: aucun</option>
<option value="left" <?php echo ($align == 'left' ? 'selected="selected"' : '');?>>Gauche</option>
<option value="right" <?php echo ($align == 'right' ? 'selected="selected"' : '');?>>Droite</option>
</select>
</p>
...
<br/>
<p>
<input type="checkbox" id="do_link" name="do_link"> Entourer l'image avec un lien
</p>
...
<option value="_self">Cible : aucune</option>
						...
						<div style="clear: both;"></div>
							...
							<div class="tab-pane" id="tab2">
								...
								<div class="pull-left" style="padding-left: 11px;">
									<button class="btn" disabled id="lib-back" rel="<?php echo LIBRARY_FOLDER_PATH; ?>"><i class="icon-hand-left"></i> Retour</button>&nbsp;&nbsp;&nbsp;<a href="" title="refresh" rel="<?php echo LIBRARY_FOLDER_PATH; ?>" id="refresh"><i class="icon-refresh"></i></a>
					
								</div>
								
								<div class="pull-right" style="padding-right: 12px;">
									<input type="text" class="input-medium" id="search" placeholder="Rechercher">
								</div>
								<?php if(CanCreateFolders()){?>
								<div class="pull-right" style="padding-right: 12px;">
									<span id="new-folder-msg"></span>
									<div class="input-append">
										<input class="input-medium" id="newfolder_name" type="text" placeholder="Créer un répertoire ici">
...
$(function(){
	...
	acceptFileTypes: /(\.|\/)(<?php echo implode("|", explode(",", ALLOWED_IMG_EXTENSIONS));?>)$/i,
        ...
        add: function (e, data) {
            ...
            var tpl = $('<li><div class="alert alert-info"><img class="loader" src="bootstrap/img/ajax-loader.gif"> <a class="close" data-dismiss="alert">×</a></div></li>');
...
<div>
<div class="pull-left">
	<p class="muted">Taille maximum du fichier uploadé : <?php echo $upload_mb;?> MB</p>
</div>
...
<form id="upload" method="post" action="upload.php" enctype="multipart/form-data">
			...
			<div id="drop">
				...
				<a class="btn">Cliquez ou déposez</a>
