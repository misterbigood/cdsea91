...
class offres_emploi
{
	...
	public $nbreOffres_emploi = '';
	public $nbreOffres_emploiParPage = 2; /* pour calcul de la pagination lors du renvoi vers le site */
    
    function __construct()
    {
		$champs = array('offres_emploi.rubrique', 'offres_emploi.actif'); 
		$tables = array('offres_emploi'); // toutes les tables filtrées 
		...
		$debutQry = 'SELECT * FROM offres_emploi'; 
		...
		/* 2ème requête pour déterminer la page de chaque offre sur le site */
		 
		$qry = "SELECT ID FROM offres_emploi WHERE actif > 0 ORDER BY date_publication DESC";
		$db = new MySQL();
		$db->Open();
		$db->query($qry);		
		$nbre = $db->RowCount();
		...
		if(!empty($nbre)) { // SI LA REQUETE N'EST PAS VIDE
			while (! $db->EndOfSeek()) {
				...
				if($index_enregistrement > $this->nbreOffres_emploiParPage * $index_page) {
					$index_page ++;
				}
				$row = $db->Row();
				$offre_ID = $row->ID;
				$this->index_page[$offre_ID] = $index_page;
			}
		}
	}  
    ...
    public function afficherTableauOffres_emploiAdmin()
    {
		$html = $this->tableauFiltres;
		$html .= '<table id="tableauOffres_emploiAdmin" class="tableAdmin centre">' . " \n"; 
		$numbersArray = array(10,20,50); 
		$html .= '<caption>liste des offres d\'emploi' . utils::selectNbreParPage($numbersArray, $_SESSION['npp']) . '</caption>' . " \n"; 
		$html .= '<thead>' . " \n"; 
		$html .= '<tr>' . " \n"; 
		if(isset($_GET['p'])) { 
		...
		$html .= '<th class="titre"><div class="tri">date de publication' . utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsUp, $arrayUrlValeursUp, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triUp'). utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsDown, $arrayUrlValeursDown, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triDown').'</div></th>' . " \n";
		if(isset($_GET['p'])) { 
			$arrayUrlVarsUp = array('tri', 'direction', 'p'); 
		...
		$html .= '<th class="titre"><div class="tri">rubrique' . utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsUp, $arrayUrlValeursUp, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triUp'). utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsDown, $arrayUrlValeursDown, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triDown').'</div></th>' . " \n";
		if(isset($_GET['p'])) { 
			$arrayUrlVarsUp = array('tri', 'direction', 'p'); 
		...
		$html .= '<th class="titre"><div class="tri">titre' . utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsUp, $arrayUrlValeursUp, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triUp'). utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsDown, $arrayUrlValeursDown, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triDown').'</div></th>' . " \n";  
		if(isset($_GET['p'])) { 
			$arrayUrlVarsUp = array('tri', 'direction', 'p'); 
		...
		$html .= '<th class="titre"><div class="tri">actif' . utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsUp, $arrayUrlValeursUp, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triUp'). utils::lienAjax('inc/afficherOffres_emploi.php', $arrayUrlVarsDown, $arrayUrlValeursDown, '<img src="images/tri.png" alt="" />', 'divOffres_emploi', 'triDown').'</div></th>' . " \n"; 
		...
		for($i=0; $i<$this->nbreOffres_emploi; $i++) { 
			...
			$offre_ID = $this->ID[$i];
			if($this->index_page[$offre_ID] < 2) $txt_index_page = '';
			else $txt_index_page = '-p' . $this->index_page[$offre_ID];
    		$html .= '<td class="' . $class.'"><a href="../offres_emploi' . $txt_index_page . '.html#offre-' . $this->ID[$i] . '" title="voir sur le site (nouvelle fen&ecirc;tre)" target="_blank"><img src="images/oeil.png" alt="voir" /></a>' . '</td>' . " \n"; 
    		$html .= '<td class="' . $class.'">'.utils::lienAjax('inc/modifOffres_emploi.php', 'ID', $this->ID[$i], '<span class="icon icon-pencil"></span>modif', 'divAjax', 'btn-orange').'</td>' . " \n"; 
    		$html .= '<td class="' . $class.'">'.utils::lienAjax('inc/supprimerOffres_emploi.php', 'ID', $this->ID[$i], '<span class="icon icon-remove"></span>suppr', 'divAjax', 'btn-red').'</td>' . " \n"; 
    		$html .= '</tr>' . " \n"; 
		} 
		$html .= '</table>' . " \n"; 
		echo $html; 
	...
	{ 
		...
		for($i=0; $i<$nbreChamps; $i++) { 
			...
			if(!empty($nbreValeurs)) { // SI LA REQUETE N'EST PAS VIDE 
				$form = new form('filtresOffres_emploi'); 
				$retour .= '<td>' . " \n"; 
				while (! $db->EndOfSeek()) { 
					$row = $db->Row(); 
					if($champs[$i] == 'scID') {  
				...
				$nomFiltre = 'offres_emploiFiltre' . preg_replace('`\.`', '_', $champs[$i]); 
	...
	{ 
		...
		if(isset($_POST['annulerFiltres'])) { 
			foreach($champs AS $champs) { 
				$champPoste = 'offres_emploiFiltre' . preg_replace('`\.`', '_', $champs); 
	...
	{
		if(isset($_GET['tri'])) $_SESSION['triOffres_emploi'] = ' '.$_GET['tri'];
		else if(!isset($_SESSION['triOffres_emploi'])) $_SESSION['triOffres_emploi'] = ' ID';
		if(isset($_GET['direction'])) $_SESSION['directionOffres_emploi'] = ' '.$_GET['direction'];
		else if(!isset($_SESSION['directionOffres_emploi'])) $_SESSION['directionOffres_emploi'] = ' DESC';
		return $_SESSION['triOffres_emploi'].$_SESSION['directionOffres_emploi'];
	} 
}
?> 