...
$messages = Array (
 "noFollow" => Array (
	0 => " <span class=\"color-red\"> lien No-follow</span>. ",
	1 => " No-follow flag set."
 ),
 "inDatabase" => Array (
	0 => " <span class=\"color-red\"> déjà dans la base de données</span><br>",
	1 => " already in database\n"
 ),
 "completed" => Array (
	0 => '<p class="centre"><span>Terminé à %cur_time.</span><p>',
	1 => "Completed at %cur_time.\n"
 ),
 "starting" => Array (
	0 => " <span>Début d'indexation à %cur_time.</span>\n",
 ...
 "pageRemoved" => Array (
	0 => " <span class=\"color-red\">Page supprimée de l'index.</span><br>\n",
	1 => " Page removed from index.\n"
 ),
  "continueSuspended" => Array (
	0 => "<br><span>poursuite de l'indexation.</span><br>\n",
	1 => "Continuing suspended indexing.\n"
 ),
  "indexed" => Array (
	0 => "<br><span class=\"color-green\">Indexé</span><br>\n",
	1 => " \nIndexed\n"
 ),
"duplicate" => Array (
	0 => " <span class=\"color-red\">Page dupliquée.</span><br>\n",
	1 => " Page is a duplicate.\n"
 ),
"md5notChanged" => Array (
	0 => " <span class=\"color-red\">somme MD5 vérifiée. page non modifiée</span><br>\n",
	1 => " MD5 sum checked. Page content not changed.\n"
 ),
"metaNoindex" => Array (
	0 => " <span class=\"color-red\">No-Index flag set in meta tags.</span><br>\n",
	1 => " No-Index flag set in meta tags.\n"
 ),
  "re-indexed" => Array (
	0 => " <span class=\"color-green\">Re-indexé</span><br>\n",
	1 => " Re-indexed\n"
 ),
"minWords" => Array (
	0 => " <span class=\"color-red\">La page contient moins de $min_words_per_page mots</span><br>\n",
...
function printRetrieving($num, $thislink, $cl) {
	...
	$log_msg_html = "<p><b>$num</b>. Réception : <b>$thislink</b> à " . date("H:i:s").".</p>\n";
	if ($print_results) {
		if ($cl==0) {
			print $log_msg_html;
		} else {
...
function printLinksReport($numoflinks, $all_links, $cl) {
	...
	$log_msg_html = "<p>Liens trouvés : <span>$all_links</span>. Nouveaux liens: <span>$numoflinks</span></p>\n";
	if ($print_results) {
		if ($cl==0) {
			print $log_msg_html;
		} else {
...
function printHeader($omit, $url, $cl) {
	...
	//$log_msg_html_1 = "<html><head><LINK REL=STYLESHEET HREF=\"css/admin.css\" TYPE=\"text/css\"></head>\n";
	//$log_msg_html_1 .= "<body style=\"font-family:Verdana, Arial; font-size:12px\">";
	$log_msg_html_1 = '';
	
	$log_msg_html_link = '<div class="centre"><a onclick="fermer(\'divAjax\');" class="btn" href="#">fermer</a></div>' . " \n";
	$log_msg_html_2 = "<h2>analyse du site $url</h2>\n";

	if (count($omit) > 0) {
		$log_msg_html_2 .=  "<p>Fichiers et répertoires interdits par le fichier robots.txt:</p>\n";
		$log_msg_html_2 .=  '<p>' . implode("<br>", $omits) . '</p>' . " \n";
	}
...
function printPageSizeReport($pageSize) {
	global $print_results, $log_format;
	$log_msg_txt = "<span>Taille de la page : $pageSize"."kb. </span>";
	if ($print_results) {
		print $log_msg_txt;
		flush();
	}
