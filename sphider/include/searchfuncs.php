	...
	function search($searchstr, $category, $start, $per_page, $type, $domain) {
		...
		if ((count($result_array_full) == 0 || $possible_to_find == 0) && $did_you_mean_enabled == 1) {
			...
			foreach ($searchstr['+'] as $word) {
				...
				mysql_query("SET NAMES 'utf8'");
				$result = mysql_query("select keyword from ".$mysql_table_prefix."keywords where soundex(keyword) = soundex('$word')"); echo "select keyword from ".$mysql_table_prefix."keywords where soundex(keyword) = soundex('$word')";
				$max_distance = 100;
				$near_word ="";
				while ($row=mysql_fetch_row($result)) {
