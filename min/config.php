...
Pour que les js soient GZIPPES, 
	1. dans le htaccess principal, ajouter application/x-javascript \  dans la liste :
	...
	2. dans le htaccess du répertoire /min/, commenter #SetEnv no-gzip
	
Si le site se trouve dans un sous-répertoire :
	-	placer le répertoire min à la racine
	-	$min_documentRoot = $_SERVER['DOCUMENT_ROOT'] . '/v2';

Le chemin vers les fichiers temporaires (cache) est défini par $min_cachePath = $_SERVER['DOCUMENT_ROOT'] . '/cache';
Si le dossier n'existe pas, min va utiliser le dossier temporaire du serveur (ex : /temp)
Si on ne le définit pas, GROS PROBLEMES pour rafraichir le cache : il faut aller vider manuellement ces fichiers sur le serveur.

Pour rafraichir le cache : $min_serveOptions['maxAge'] = 0;
Pour forcer le fafraichissement en cas de problème : 
	vider le dossier $_SERVER['DOCUMENT_ROOT'] . '/cache'
	OU ajouter un nombre aléatoire en fin de requête (ex : <link type="text/css" rel="stylesheet" href="/min/b=css&amp;f=normalize.css,main.css&1" />
...
$min_cachePath = $_SERVER['DOCUMENT_ROOT'] . '/cache';
 ...
 */
$min_serveOptions['maxAge'] = 2592000; /* 2592000 = 30 jours */

/* =============================================
	AJOUT POUR LOCALHOST ( => http://code.google.com/p/minify/wiki/AlternateFileLayouts )
   ============================================= */
...
// Set the document root to be the path of the "site root"
	...
	/* 
		...
		activer $min_enableBuilder + les lignes ci-dessous (puis désactiver sinon les chemins ne sont plus bons.)
	*/
	//$min_documentRoot = str_replace('\ecoc', '', $min_documentRoot);
...
/* =============================================
	FIN AJOUT POUR LOCALHOST
   ============================================= */

$min_serveOptions['rewriteCssUris'] = true; // pour les chemins des images en css ()
