...
class MySQL
{
	...
	private $db_host    = "mysql51-111.perso";  // server name
	private $db_user    = "cdseabddsql";       // user name
	private $db_pass    = "uTxbklgUO";           // password
	private $db_dbname  = "cdseabddsql";      // database name
	private $db_host_local    = "localhost";  // server name
	private $db_user_local    = "root";       // user name
	private $db_pass_local    = "";           // password
	private $db_dbname_local  = "cdsea";      // database name
	...
	public function __construct($connect = true, $database = null, $server = null,
		...
		if($_SERVER['SERVER_NAME'] == 'localhost') {
			$this->db_host    = $this->db_host_local;
			$this->db_user    = $this->db_user_local;
			$this->db_pass    = $this->db_pass_local;
			$this->db_dbname  = $this->db_dbname_local;
		}
			...
			case "array":
				...
				foreach ($columns as $key => $value) {
					...
					if (strlen($sql) == 0) {
						...
						if($value<>'') $sql .= ", " . $quote . $value . $quote;
                                                else $sql .= ", ''";
	...
	*/
	...
	public function InsertRow($tableName, $valuesArray) {
		...
		if (! $this->IsConnected()) {
			...
			// Execute the query
			echo $sql = self::BuildSQLInsert($tableName, $valuesArray);
